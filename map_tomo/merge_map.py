...
output = {
    ...
    'json':'IOfile/json/packing{}.json'.format(num)}
...
if __name__ == '__main__':
    ...
    import mrcfile
    import iomap as IM
    rootdir = '/ldap_shared/home/v_sinuo_liu/mayan/aitom/mapfile'
    v = IM.readMrcMapDir(rootdir)

    # get packing info
    import packing.simulate as SI
    packing_result = SI.packing_with_target(target_protein='1bxn', random_protein_number=4)
    ...
    x0 = packing_result['optimal_result']['initialization'][0]/10
    y0 = packing_result['optimal_result']['initialization'][1]/10
    z0 = packing_result['optimal_result']['initialization'][2]/10
    box_size = packing_result['general_info']['box_size']/10
    # save packing info
    with open(output['json'],'w') as f:
        json.dump(packing_result, f, cls=NumpyEncoder)

    # merge map to hugemap
    initmap = merge_map(v, protein_name, x0, y0, z0, box_size)
    packmap = merge_map(v, protein_name, x, y, z, box_size)
    IM.map2mrc(initmap, output['initmap']['mrc'])
    IM.map2mrc(initmap, output['packmap']['mrc'])
