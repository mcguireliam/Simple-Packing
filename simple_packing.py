...
#       xx = x[kk] + ball_radius[kk] * np.outer(np.cos(u), np.sin(v)) 
#       yy = y[kk] + ball_radius[kk] * np.outer(np.sin(u), np.sin(v)) 
#       zz = z[kk] + ball_radius[kk] * np.outer(np.ones(np.size(u)), np.cos(v))
